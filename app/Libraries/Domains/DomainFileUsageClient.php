...
/**
 * Calls the internal file routes exposed by a configured domain app.
 * Requests are HMAC-signed because the Hub cannot replay a domain API key
 * that is stored as a one-way hash.
 */
    ...
    public function collectUsages(int $fileId): array
    {
        ...
        foreach ($this->config->domains as $code => $baseUrl) {
            try {
                ...
                log_message(
                    'warning',
                    "[DomainFileUsageClient] usage check failed for domain '{$code}' (file {$fileId}): " . $e->getMessage()
                );
            }
        }
    ...
    public function broadcastInvalidate(int $fileId): void
    {
        ...
        foreach ($this->config->domains as $code => $baseUrl) {
            try {
                ...
                log_message(
                    'warning',
                    "[DomainFileUsageClient] invalidate-cache failed for domain '{$code}' (file {$fileId}): " . $e->getMessage()
                );
            }
        }
    }
